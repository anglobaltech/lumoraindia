...
import { X, Package, MapPin, Calendar, Receipt, Truck, CheckCircle, Loader2, XCircle } from "lucide-react";
...
export default function OrdersPage() {
    ...
    const fetchMyOrders = async () => {
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          fetchedOrders.push({
            docId: doc.id, // IMPORTANT: Needed to update the document in Firestore
            id: data.orderId,
            date: data.createdAt 
  ...
  const handleCancelOrder = async (orderToCancel) => {
    try {
      // 1. Update the order status in Firestore (this syncs it with the Admin Panel)
      ...
      // 2. Call the API to send the Cancellation Email
      ...
      // 3. Update the UI locally so the user sees it immediately
      setOrders(prevOrders => 
        prevOrders.map(o => o.id === orderToCancel.id ? { ...o, status: "Cancelled" } : o)
      );

      // If the modal is currently open for this order, update its status too
              ...
              orders.map((order) => (
                <div key={order.id} className="group transition-all duration-300 hover:-translate-y-1">
                  <OrderCard 
                    ...
                    onCancel={handleCancelOrder} // PASSED CANCEL FUNCTION TO CARD
                  />
                </div>
              ))
            ) : (
