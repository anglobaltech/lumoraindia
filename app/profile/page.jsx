...
export default function ProfilePage() {
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className="text-gray-500 font-semibold animate-pulse text-lg tracking-wide">Loading your premium profile...</p>
      </div>
    );
  }
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // Using setDoc with { merge: true } safely creates the document if it's missing!
  ...
  const EmptyField = ({ label }) => (
    <button
      ...
      className="text-sm font-semibold text-pink-500 hover:text-pink-600 flex items-center gap-2 transition-all cursor-pointer group w-full text-left"
    >
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] p-6 sm:p-8 lg:p-10 shadow-xl shadow-pink-100/50 border border-pink-100 relative overflow-hidden">
        ...
        <form onSubmit={handleSave}>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-5 border-b border-pink-100/50 pb-6">
            <div>
              <h2 className="text-xl sm:text-2xl font-semibold text-gray-700 tracking-tight">Personal Details</h2>
              <p className="text-gray-500 text-sm mt-1.5 font-medium">Manage your personal information and delivery address.</p>
            </div>
            
            <div className="flex items-center w-full sm:w-auto">
              {!isEditing ? (
                <button type="button" onClick={() => setIsEditing(true)} className="cursor-pointer w-full sm:w-auto px-8 py-3 bg-pink-50 text-pink-600 rounded-xl text-sm font-semibold hover:bg-pink-100 hover:shadow-sm transition-all duration-300">
                  Edit Profile
                </button>
              ) : (
                <div className="flex gap-3 w-full sm:w-auto">
                  <button type="button" onClick={handleCancel} disabled={isSaving} className="cursor-pointer flex-1 sm:flex-none px-6 py-3 bg-white text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-50 hover:text-gray-700 transition-all flex items-center justify-center gap-2 border-2 border-gray-200">
                    <X size={16}/> Cancel
                  </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1.5">
              ...
              <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all duration-300 ${isEditing ? 'bg-white border-pink-200 focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-100 shadow-sm' : 'bg-gray-50/50 border-gray-100 hover:bg-gray-50'}`}>
                ...
                {isEditing ? (
                  <input type="text" placeholder="e.g. John Doe" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="bg-transparent w-full outline-none text-gray-700 text-base font-semibold placeholder-gray-300" />
                ) : (
                  (user?.name || profile?.name) 
                    ? <span className="font-semibold text-base text-gray-700">{user?.name || profile?.name}</span> 
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest ml-1">Email Address</label>
            ...
            {/* Phone */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest ml-1">Phone Number</label>
              <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all duration-300 ${isEditing ? 'bg-white border-pink-200 focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-100 shadow-sm' : 'bg-gray-50/50 border-gray-100 hover:bg-gray-50'}`}>
                <Phone size={20} className={isEditing ? 'text-pink-500' : 'text-gray-400'} />
                {isEditing && <span className="text-gray-400 font-semibold text-base border-r border-gray-200 pr-2">+91</span>}
                {isEditing ? (
                  <input type="tel" placeholder="9876543210" maxLength={10} value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value.replace(/\D/g, '')})} className="bg-transparent w-full outline-none text-gray-700 text-base font-semibold placeholder-gray-300" />
                ) : (
                  (user?.phone || profile?.phone)
                    ? <span className="font-semibold text-base text-gray-700">+91 {user?.phone || profile?.phone}</span>
            ...
            {/* DOB */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest ml-1">Date of Birth</label>
              <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all duration-300 ${isEditing ? 'bg-white border-pink-200 focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-100 shadow-sm' : 'bg-gray-50/50 border-gray-100 hover:bg-gray-50'}`}>
                <Calendar size={20} className={isEditing ? 'text-pink-500' : 'text-gray-400'} />
                {isEditing ? (
                  <input type="date" value={formData.dob} onChange={(e) => setFormData({...formData, dob: e.target.value})} className="bg-transparent w-full outline-none text-gray-700 text-base font-semibold cursor-pointer" />
                ) : (
                  (user?.dob || profile?.dob)
                    ? <span className="font-semibold text-base text-gray-700">{user?.dob || profile?.dob}</span>
            ...
            <div className="space-y-1.5 md:col-span-2">
              <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest ml-1">Gender</label>
                ...
                {isEditing ? (
                  <>
                    <select 
                      ...
                      className="bg-transparent w-full outline-none text-gray-700 text-base font-semibold cursor-pointer appearance-none z-10 relative"
                    >
                  ...
                  (user?.gender || profile?.gender)
                    ? <span className="font-semibold text-base text-gray-700">{user?.gender || profile?.gender}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className={`relative flex items-center px-4 py-3.5 rounded-xl border-2 transition-all duration-300 ${isEditing ? 'bg-white border-pink-200 focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-100 shadow-sm' : 'bg-white border-gray-100 hover:border-pink-100'}`}>
                      {isEditing ? (
                        <input type="text" maxLength={6} placeholder="e.g. 110001" value={formData.pincode} onChange={handlePincodeChange} className="bg-transparent w-full outline-none text-gray-700 text-base font-semibold placeholder-gray-300" />
                      ) : (
                        (user?.pincode || profile?.pincode)
                          ? <span className="text-base font-semibold text-gray-700">{user?.pincode || profile?.pincode}</span>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all duration-300 ${isEditing ? 'bg-white border-pink-200 focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-100 shadow-sm' : 'bg-white border-gray-100 hover:border-pink-100'}`}>
                      ...
                      {isEditing ? (
                        <input type="text" placeholder="Flat 401, Building B" value={formData.houseNo} onChange={(e) => setFormData({...formData, houseNo: e.target.value})} className="bg-transparent w-full outline-none text-base text-gray-700 font-semibold placeholder-gray-300" />
                      ) : (
                        (user?.houseNo || profile?.houseNo)
                          ? <span className="text-base font-semibold text-gray-700">{user?.houseNo || profile?.houseNo}</span>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all duration-300 ${isEditing ? 'bg-white border-pink-200 focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-100 shadow-sm' : 'bg-white border-gray-100 hover:border-pink-100'}`}>
                      ...
                      {isEditing ? (
                        <input type="text" placeholder="Sector 62" value={formData.area} onChange={(e) => setFormData({...formData, area: e.target.value})} className="bg-transparent w-full outline-none text-base text-gray-700 font-semibold placeholder-gray-300" />
                      ) : (
                        (user?.area || profile?.area)
                          ? <span className="text-base font-semibold text-gray-700">{user?.area || profile?.area}</span>
                  ...
                  {/* Landmark */}
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest ml-1">Landmark (Optional)</label>
                    <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all duration-300 ${isEditing ? 'bg-white border-pink-200 focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-100 shadow-sm' : 'bg-white border-gray-100 hover:border-pink-100'}`}>
                      <Landmark size={18} className={isEditing ? 'text-pink-400' : 'text-gray-300'} />
                      {isEditing ? (
                        <input type="text" placeholder="Near Apollo Hospital" value={formData.landmark} onChange={(e) => setFormData({...formData, landmark: e.target.value})} className="bg-transparent w-full outline-none text-base text-gray-700 font-semibold placeholder-gray-300" />
                      ) : (
                        (user?.landmark || profile?.landmark)
                          ? <span className="text-base font-semibold text-gray-700">{user?.landmark || profile?.landmark}</span>
                  ...
                  {/* City */}
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest ml-1">City / District</label>
                    <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all duration-300 ${isEditing ? 'bg-white border-pink-200 focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-100 shadow-sm' : 'bg-white border-gray-100 hover:border-pink-100'}`}>
                      <Map size={18} className={isEditing ? 'text-pink-400' : 'text-gray-300'} />
                      {isEditing ? (
                        <input type="text" placeholder="New Delhi" value={formData.city} onChange={(e) => setFormData({...formData, city: e.target.value})} className="bg-transparent w-full outline-none text-base text-gray-700 font-semibold placeholder-gray-300" />
                      ) : (
                        (user?.city || profile?.city)
                          ? <span className="text-base font-semibold text-gray-700">{user?.city || profile?.city}</span>
        ...
        <div className="mt-10 pt-8 border-t border-pink-100/50 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="max-w-md">
            <h4 className="text-base font-semibold text-gray-700">Sign Out</h4>
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 px-8 py-3.5 bg-red-50 text-red-600 font-semibold rounded-xl border border-red-100 hover:bg-red-600 hover:text-white transition-all duration-300 cursor-pointer group shadow-sm w-full md:w-auto hover:shadow-md"
          >
