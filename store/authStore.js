...
import { create } from 'zustand';
import { auth, db } from '../lib/firebase'; // Removed adminAuth import
import {
    createUserWithEmailAndPassword,
    signInWithEmailAndPassword,
    signOut,
...
export const useAuthStore = create((set, get) => ({
    ...
    // 5. Admin Login (Fixed Database Race Condition)
    adminLogin: async (email, password) => {
        set({ adminLoading: true, error: null });
        try {
            // 1. Authenticate using the default auth so the database recognizes the session
            const userCredential = await signInWithEmailAndPassword(auth, email, password);
            ...
            if (adminSnap.exists() && adminSnap.data().role === "admin") {
                ...
                // If they logged in but aren't in the admin collection, kick them out
                await signOut(auth);
                set({ adminUser: null, adminProfile: null, error: "Unauthorized access", adminLoading: false });
                return { success: false, error: "Unauthorized access. Staff only." };
            }
    ...
    // 7. Combined Auth Listener (Unified & Conflict-Free)
    initAuthListener: () => {
        const unsub = onAuthStateChanged(auth, async (user) => {
            if (user) {
                set({ user: user, loading: true, adminLoading: true });
                
                try {
                    // Fetch both profiles simultaneously to save time
                    ...
                    // Update Admin State
                    if (adminSnap.exists() && adminSnap.data().role === "admin") {
                        set({ adminProfile: adminSnap.data(), adminUser: user, adminLoading: false });
                    } else {
                        set({ adminProfile: null, adminUser: null, adminLoading: false });
                    }

                } catch (err) {
                    set({ profile: null, adminProfile: null, loading: false, adminLoading: false });
                }
            } else {
                // Completely reset on logout
                set({ 
                    user: null, profile: null, loading: false,
                    adminUser: null, adminProfile: null, adminLoading: false 
                });
            }
        });
